...
import { sanitizeAnalysisSubmission } from "@/lib/analysis/sanitize";
...
export function InputPanel({ isAnalyzing, locale, copy, onAnalyze }: Props) {
  ...
  const handleAnalyze = () => {
    ...
    const safeSubmission = sanitizeAnalysisSubmission({
      cvText,
      jobDescription: jdMode === "paste" ? jdText : undefined,
      jobUrl: jdMode === "url" ? jdUrl : undefined,
    });
    onAnalyze({
      cvText: safeSubmission.cvText,
      jobDescription: safeSubmission.jobDescription,
      jobUrl: safeSubmission.jobUrl,
    });
  };
