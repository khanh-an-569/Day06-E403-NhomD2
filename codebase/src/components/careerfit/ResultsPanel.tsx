...
function ResultsView({
  ...
  return (
    <div className="space-y-6">
      ...
      {(result.persona || result.jdAnalysis) && (
        <Card className="p-6" style={{ boxShadow: "var(--shadow-card)" }}>
          <div className="mb-4 flex items-center gap-2">
            <Target className="h-4 w-4 text-primary" />
            <h3 className="text-sm font-semibold text-foreground">
              {locale === "en" ? "JD Persona & Highlights" : "Persona & Điểm nổi bật của JD"}
            </h3>
          </div>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-border/60 bg-secondary/20 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">
                {locale === "en" ? "Persona" : "Persona"}
              </p>
              <p className="mt-1 text-sm font-medium text-foreground">
                {result.persona?.role || result.jobTitle || "—"}
              </p>
              <div className="mt-3 space-y-2 text-sm text-foreground">
                <p>
                  ...
                  {result.persona?.level || "unknown"}
                </p>
                <p>
                  <span className="font-medium text-muted-foreground">
                    {locale === "en" ? "Domain" : "Ngành"}:
                  </span>{" "}
                  {result.persona?.domain || "—"}
                </p>
              </div>
            </div>
            <div className="space-y-3">
              <div className="rounded-xl border border-border/60 bg-secondary/20 p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {(result.jdAnalysis?.mandatory ?? []).map((item) => (
              ...
              <div className="rounded-xl border border-border/60 bg-secondary/20 p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {(result.jdAnalysis?.bonus ?? []).map((item) => (
          ...
          {(result.jdAnalysis?.highlights.length ?? 0) > 0 && (
            <div className="mt-4 rounded-xl border border-border/60 bg-secondary/20 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">
                {locale === "en" ? "JD Highlights" : "Điểm cần chú ý trong JD"}
              </p>
              <ul className="mt-2 space-y-2 text-sm text-foreground">
                {result.jdAnalysis?.highlights.map((item) => (
      ...
      {(result.cvAnalysis || result.jdAnalysis) && (
        <Card className="p-6" style={{ boxShadow: "var(--shadow-card)" }}>
          <div className="mb-4 flex items-center gap-2">
            <Link2 className="h-4 w-4 text-primary" />
            <h3 className="text-sm font-semibold text-foreground">
              {locale === "en" ? "CV vs JD Comparison" : "So sánh CV và JD"}
            </h3>
          </div>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">
                {locale === "en" ? "JD base" : "Base JD"}
              </p>
              <div className="mt-2 space-y-4">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {locale === "en" ? "Highlights" : "Điểm nổi bật"}
                  </p>
                  <ul className="mt-2 space-y-2 text-sm text-foreground">
                    {(result.jdAnalysis?.highlights ?? []).map((item) => (
                      <li key={item} className="flex gap-2">
            ...
            <div className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">
                {locale === "en" ? "CV evidence" : "Bằng chứng CV"}
              </p>
              <div className="mt-2 space-y-4">
                <div>
                  <p className="text-sm font-medium text-foreground">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {(result.cvAnalysis?.skills ?? []).map((item) => (
                      <Badge key={item} variant="secondary" className="rounded-full">
                        {item}
                      </Badge>
                    ))}
                ...
                <div>
                  ...
                  <ul className="mt-2 space-y-2 text-sm text-foreground">
                    {(result.cvAnalysis?.experience ?? []).map((item) => (
                      <li key={item} className="flex gap-2">
                        <span className="text-primary">•</span>
                        <span>{item}</span>
                      </li>
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {locale === "en" ? "Projects & Metrics" : "Dự án & số liệu"}
                  </p>
                  <ul className="mt-2 space-y-2 text-sm text-foreground">
                    {[...(result.cvAnalysis?.projects ?? []), ...(result.cvAnalysis?.metrics ?? [])].map((item) => (
                      <li key={item} className="flex gap-2">
                        <span className="text-primary">•</span>
                        <span>{item}</span>
                      </li>
      ...
      {result.criteria && result.criteria.length > 0 && (
        <Card className="p-6" style={{ boxShadow: "var(--shadow-card)" }}>
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">
              {locale === "en" ? "Criteria Details" : "Chi tiết tiêu chí"}
            </h3>
          </div>
          <div className="space-y-4">
            {result.criteria.map((criterion) => (
              <div key={criterion.name} className="rounded-xl border border-border/60 bg-secondary/20 p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{criterion.name}</p>
                    <p className="text-xs text-muted-foreground">
                      {criterion.status} · {criterion.score}/100
                    </p>
                  </div>
                  <div className="text-sm font-semibold text-foreground">{criterion.score}/100</div>
                </div>
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      {locale === "en" ? "JD requirement" : "Yêu cầu JD"}
                    </p>
                    <ul className="mt-1 space-y-1 text-sm text-foreground">
                      {criterion.jdRequirement.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="text-primary">•</span>
                          <span>{item}</span>
                        </li>
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      {locale === "en" ? "CV evidence" : "Bằng chứng từ CV"}
                    </p>
                    <ul className="mt-1 space-y-1 text-sm text-foreground">
                      {criterion.cvEvidence.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="text-primary">•</span>
                          <span>{item}</span>
                        </li>
                ...
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      {locale === "en" ? "Improve" : "Cải thiện"}
                    </p>
                    <ul className="mt-1 space-y-1 text-sm text-foreground">
                      {criterion.improvement.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="text-[oklch(0.65_0.16_75)]">•</span>
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {criterion.note && (
                  <p className="mt-3 text-xs text-muted-foreground">{criterion.note}</p>
                )}
              </div>
            ))}
          </div>
